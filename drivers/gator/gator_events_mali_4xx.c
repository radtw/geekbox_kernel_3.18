...
#if TSAI /* temporary for IMG */
static unsigned int n_fp_cores = 4;
#else
static unsigned int n_fp_cores = MAX_NUM_FP_CORES;
#endif

#if TSAI
extern struct mali_counter mali_activity[4];
#else
...
#if TSAI
unsigned tsai_initialise_version_info;
#endif
...
#if TSAI
		tsai_initialise_version_info = 1;
#endif
	} else {
...
#if TSAI
	mali_activity[2].cores = 1;
	mali_activity[3].cores = 1;
#endif
...
#if 0 && TSAI /* not needed I don't provide a counter value for that */
	/* TQ3D */
	{
		int activity_counter_id = ACTIVITY_TQ3D;

		snprintf(buf, sizeof(buf), "ARM_Mali-%s_TQ3D_%d_active", mali_name, 0);
		if (create_fs_entry(sb, root, buf, activity_counter_id, 0) != 0)
			return -1;

		for (counter_number = 0; counter_number < 2; counter_number++) {
			int counter_id = COUNTER_TQ3D_0_C0 + counter_number;

			snprintf(buf, sizeof(buf), "ARM_Mali-%s_TQ3D_%d_cnt%d", mali_name, 0, counter_number);
			if (create_fs_entry(sb, root, buf, counter_id, 1) != 0)
				return -1;
		}
	}
...
#if TSAI
	if (!tsai_initialise_version_info) {
...
int gator_events_mali_init(void)
{
...
#if TSAI && defined(CONFIG_OF)
	{
		...
		node = of_find_compatible_node(NULL, NULL, "samsung,dtv");
		of_property_read_string(node, "model", &out_model);
		if (out_model && strcmp(out_model, "Samsung DTV based on SDP1701(Kant-M2) SoC")==0) {
			n_fp_cores = 1;
		}
	}
#endif
