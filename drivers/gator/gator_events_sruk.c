...
struct sruk_mali_mem_prof_streamline {
	unsigned version; /* version of this structure */
	unsigned size; /* size of this structure */
	unsigned mali_usage; /* current mali used memory */
	unsigned mali_limit; /* current mali limitation */
};

/* streamline may call this function to retrieve memory statistics info */
typedef void (*ptr_sruk_mali_mem_prof_streamline_read) (struct sruk_mali_mem_prof_streamline* info);
ptr_sruk_mali_mem_prof_streamline_read sruk_mali_mem_prof_streamline_read;
					     ...
					     struct dentry *root)
{
	...
	for (i = 0; i < sruk_COUNTERS_NUM; i++) {
		...
		snprintf(buf, sizeof(buf), "sruk_cnt%d", i);
...
static int gator_events_sruk_start(void)
{
	...
	if (!sruk_mali_mem_prof_streamline_read) {
		sruk_mali_mem_prof_streamline_read = (ptr_sruk_mali_mem_prof_streamline_read)symbol_get(sruk_mali_mem_prof_streamline_read);
	}
...
{
	int len = 0;
	struct sruk_mali_mem_prof_streamline mali_info;
...
int gator_events_sruk_init(void)
{
	int i;
