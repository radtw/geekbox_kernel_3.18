...
#if TSAI_USE_MAIN2 && !TSAI_IS_MAIN2 //TSAI: make the instance go to gator_main2.c
...
#if TSAI /* make it 512KB */
	#define ANNOTATE_BUFFER_SIZE      (512*1024)	/* annotate counters have the core as part of the data and the core value in the frame header may be discarded */
#else
...
#if TSAI /* make non-static */
unsigned long gator_started;
#else
...
#if TSAI
/* non-static */
DECLARE_WAIT_QUEUE_HEAD(gator_annotate_wait);
#else
 ...
 ******************************************************************************/
#if TSAI
u64 gator_get_time_from_timestamp(u64 timestamp);
#endif
...
#if TSAI
DEFINE_PER_CPU(char *[NUM_GATOR_BUFS], gator_buffer);
#else
...
#define GATOR_EVENTS_LIST \
	...
	GATOR_EVENT(gator_events_sruk_init) \
	GATOR_EVENT(gator_events_sruk_malikctxmem_init) \
	GATOR_EVENT(gator_events_sruk_mmc_init) \
	GATOR_EVENT(gator_events_srukgem_init) \
...
u64 gator_get_timestamp_now(void) {
	struct timespec ts;
	u64 timestamp;
	getrawmonotonic(&ts);
...
EXPORT_SYMBOL(gator_get_timestamp_now);


/* for delayed timestamp, just return 0 */
u64 gator_get_time_from_timestamp(u64 timestamp)
{
//	struct timespec ts;
	//u64 timestamp;
	 ...
	 */
#if 0 /* TSAI: 2017-05-26, observed unknown heavy kernel load and annotation out of order, so remove timestamp recording for IMG GPU case */
#if 1 /* retrospective timestamp is allowed in this case! */
	...
	/* if a delayed a timestamp is meant for prior to capture? */
	if (gator_monotonic_started > timestamp) {
		return 0;
	}
...
/* exported, so IMG ko can know how to obtain a relative timestamp */
EXPORT_SYMBOL(gator_get_time_from_timestamp);

/* export this function so other part of kernel can know whether ds5 is running */
unsigned int tsai_gator_started(void) {
	return gator_started;
}

EXPORT_SYMBOL(tsai_gator_started);
...
static ssize_t userspace_buffer_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	int written = 0;
#if TSAI_FTRACE
	struct timeval debug_tv_pre;
	struct timeval debug_tv_wait;
	struct timeval debug_tv_post;
	unsigned debug_wait_durtation;
	unsigned debug_shedule_duration;
#endif
	...
	debug_wait_durtation = tsai_diff_microsec(&debug_tv_pre, &debug_tv_wait);
	debug_shedule_duration = tsai_diff_microsec(&debug_tv_pre, &debug_tv_post);
	trace_printk("ret %d user size %d wait_ev %d dur %d \n", (written > 0 ? written : -EFAULT), (int) count, debug_wait_durtation, debug_shedule_duration );
#endif
...
#define GATOR_TRACEPOINTS \
	GATOR_HANDLE_TRACEPOINT(tsai_sdp_unzip_start); \
	GATOR_HANDLE_TRACEPOINT(tsai_sdp_unzip_isr); \
	GATOR_HANDLE_TRACEPOINT(tsai_sdp_mmch_request); \
	GATOR_HANDLE_TRACEPOINT(tsai_sdp_mmch_isr); \
	GATOR_HANDLE_TRACEPOINT(sched_wakeup); \
	GATOR_HANDLE_TRACEPOINT(block_rq_issue); \
	...
	GATOR_HANDLE_TRACEPOINT(mali_timeline_event_timestamp);
...
{
#if 0 && TSAI
	printk("gator_save_tracepoint name %s \n", tp->name);
#endif
...
#if TSAI
	void manually_load_mali_tracepoint_symbols(void) {
		void* ptr;
#define GATOR_HANDLE_TRACEPOINT(probe_name) \
	    if (!gator_tracepoint_##probe_name) {\
		    ptr = __symbol_get("__tracepoint_"#probe_name); \
		    if (!ptr)  \
			    printk("symobl does not exist " #probe_name"\n"); \
		    gator_tracepoint_##probe_name = (struct tracepoint*)ptr;\
	    }
...
/* examples:
		ptr = symbol_get(__tracepoint_mali_timeline_event);
		gator_tracepoint_mali_timeline_event = ptr;
*/
	}
...
#if TSAI_USE_MAIN2 && !TSAI_IS_MAIN2
int tsai_gator_module_init(void);
void tsai_gator_module_exit(void);
#endif

#if TSAI_USE_MAIN2 && TSAI_IS_MAIN2
int __init tsai_gator_module_init(void)
#else
static int __init gator_module_init(void)
#endif
{
#if TSAI_USE_MAIN2 && !TSAI_IS_MAIN2
	return tsai_gator_module_init();
#else
	for_each_kernel_tracepoint(gator_save_tracepoint, NULL);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0) && TSAI
	/* second chance, load the symbol if gator_save_tracepoint cannot find it */
	manually_load_mali_tracepoint_symbols();
#if defined(CONFIG_MODULES)
...
#if TSAI_USE_MAIN2 && TSAI_IS_MAIN2
void __exit tsai_gator_module_exit(void)
#else
static void __exit gator_module_exit(void)
#endif
{
#if TSAI_USE_MAIN2 && !TSAI_IS_MAIN2
	tsai_gator_module_exit();
#else /* original or main */
