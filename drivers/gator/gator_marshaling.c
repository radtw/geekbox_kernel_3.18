...
static void marshal_activity_switch_timestamp(int core, int key, int activity, int pid, int state, u64 timestamp)
{
	unsigned long cpu = get_physical_cpu(), flags;
	u64 time;
...
void gator_marshal_activity_switch_timestamp(int core, int key, int activity, int pid, u64 timestamp)
{
	/* state is reserved for cpu use only */
	marshal_activity_switch_timestamp(core, key, activity, pid, 0, timestamp);
}
