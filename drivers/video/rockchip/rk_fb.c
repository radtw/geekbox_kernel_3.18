			     ...
			     struct rk_fb_reg_data *regs)
{
	...
	int pagefault = 0;
#if TSAI_DS5
	ANNOTATE_CHANNEL_COLOR(1, ANNOTATE_WHITE, "rk_fb_update_reg");
#endif
...
#if TSAI
unsigned int tsai_rk_fb_set_win_buffer_seqno;
#endif
...
#if TSAI_DS5
				{
					unsigned int seqno = ion_handle_get_tsai_seqno(hdl);
				SRUK_ANNOTATE_CHANNEL_COLOR(1, ANNOTATE_LTGRAY, "rk_fb_set_win_buffer ion fd %d buf %d",
						ion_fd, seqno);
				}
#endif
				...
				struct rk_fb_win_cfg_data *win_data)
{
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			...
			{
#if TSAI_DS5
				struct rk_fb_area_par* ap = &win_data->win_par[i].area_par[j];

				SRUK_ANNOTATE_CHANNEL_COLOR(1, ANNOTATE_WHITE, "ion_fd %d xact %d yact %d",
						ap->ion_fd, ap->xact, ap->yact);
#endif
