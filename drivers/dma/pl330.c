...
#if TSAI /* To fix a crash!, dev data is needed before pl330_add() */
...
#if 1 && TSAI /* expand macro for debug */
...
pl330_driver_init(void) {
	int ret;
	ret = amba_driver_register(&(pl330_driver));
	return ret;
}

static initcall_t __initcall_pl330_driver_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = pl330_driver_init; ;;
...
pl330_driver_exit(void) {
	amba_driver_unregister(&(pl330_driver));
}

static exitcall_t __exitcall_pl330_driver_exit __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = pl330_driver_exit;;;
