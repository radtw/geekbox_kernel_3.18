...
#if 0 /* TSAI: to make compiler happy */
static IMG_BOOL rk33_dvfs_event(RGXFWIF_GPU_UTIL_STATS* psUtilStats)
{
    ...
    if (platform->debug_level == DBG_HIGH)
        printk("GPU util info:valid[%d],ActiveHigh[%lld],ActiveLow[%lld],Blocked[%lld],Idle[%lld]\n",
               psUtilStats->bValid,
               psUtilStats->ui64GpuStatActiveHigh, psUtilStats->ui64GpuStatActiveLow,
               psUtilStats->ui64GpuStatBlocked, psUtilStats->ui64GpuStatIdle);
...
#if 1 /* TSAI: worry about this function later */
	(void)psDevInfo;
#else
